...
export class JupyterServerSelector {
    ...
    public async addRemoteUri(userURI: string, ignoreValidation?: boolean, displayName?: string): Promise<void> {
        // Double check this server can be connected to. Might need a password, might need a allowUnauthorized
        try {
            if (!ignoreValidation) {
        ...
        await this.serverUriStorage.addUri(userURI, connection.displayName);

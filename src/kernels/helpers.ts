...
export function createInterpreterKernelSpecWithName(
    ...
    const interpreterMetadata = interpreter
        ? {
              path: getFilePath(getEnvironmentExecutable(interpreter))
          }
        : {};
...
export function getKernelId(spec: IJupyterKernelSpec, interpreter?: { id: string }, serverId?: string) {
    // Non-Python kernels cannot contain an interpreter (even in their id).
    const interpreterUri = interpreter && isPythonKernelSpec(spec) ? getEnvironmentExecutable(interpreter) : undefined;
    ...
    const interpreterPath = getFilePath(getNormalizedInterpreterPath(interpreterUri)) || '';
 ...
 */
export async function getInterpreterKernelSpecName(interpreter?: { id: string }): Promise<string> {
